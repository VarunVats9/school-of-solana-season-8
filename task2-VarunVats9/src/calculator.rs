...
impl Calculator {
    ...
    pub fn addition(&mut self, x: i64, y: i64) -> Option<i64> {
        match OperationType::Addition.perform(x, y) {
            Some(result) => {
    ...
    pub fn subtraction(&mut self, x: i64, y: i64) -> Option<i64> {
        match OperationType::Subtraction.perform(x, y) {
            Some(result) => {
    ...
    pub fn multiplication(&mut self, x: i64, y: i64) -> Option<i64> {
        match OperationType::Multiplication.perform(x, y) {
            Some(result) => {
    ...
    pub fn show_history(&self) -> String {
        ...
        for (index, operation) in self.history.iter().enumerate() {
            history_string.push_str(&format!("{}: {} {} {} = {}\n", index, operation.first_num, operation.operation_type.get_sign(), operation.second_num, operation.operation_type.perform(operation.first_num, operation.second_num).unwrap()));
        }
        history_string
    }
    ...
    pub fn repeat(&mut self, operation_index: usize) -> Option<i64>{
        ...
        match operation.operation_type.perform(operation.first_num, operation.second_num) {
            Some(result) => {
                self.history.push(Operation::new(operation.first_num, operation.second_num, operation.operation_type));
                return Some(result)
            }
            None => return None
        }
